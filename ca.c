...
static void generate_ca_certificate(
    gnutls_x509_crt_t* cert, gnutls_x509_privkey_t* key, gnutls_sec_param_t parameter
){
...
static void generate_ca(){
    ...
    while(true){
        if(getcwd(path, length) != NULL) break;
        if(errno != ERANGE){
...
static bool load_ca(){
    if(access(CACertificateName, R_OK) != 0 || access(CAKeyName, R_OK)) return false;
    gnutls_datum_t buffer;
