...
static void established_connection_deleter_(void* target_){
    ...
    fprintf(stderr, "eliminating established connection with %s\n", target->local_keypair->hostname);
    if(target->status >= ConnectionStatusRemoteHandshake) gnutls_deinit(target->remote_session);
