...
void                    proceed_connection(const struct dns_cache_entry *entry, ListNode *connection_node) {
  ...
  inet_ntop(
    entry->ai_family,
    entry->ai_family == AF_INET ? (void *)&(((struct sockaddr_in *)entry->ai_addr)->sin_addr)
  ...
  if (rtv == 0 || (rtv == -1 && errno == EINPROGRESS)) {
    ...
    write(pipe_to_next, &connection_node, sizeof(ListNode *));
  } else {
    fprintf(stderr, "outbound: failed when connecting to %s: %s\n", entry->hostname, strerror(errno));
...
void callback(void *arg, int status, [[maybe_unused]] int timeouts, struct ares_addrinfo *result) {
  ...
  if (status == ARES_SUCCESS) {
    struct dns_cache_entry *entry = dns_cache_insert(
      &cache, parameter->hint, parameter->hint_type, c_connection->local_keypair->hostname, result->nodes
    );
...
void *outbound_entry(void *arg) {
  ...
  struct ares_addrinfo_hints hints = {
    .ai_family = AF_UNSPEC, .ai_flags = ARES_AI_NOSORT, .ai_protocol = 0, .ai_socktype = SOCK_STREAM
  };
  ...
  while (loop) {
    ...
    FD_SET(rpc_fd, &rset);
    if (fd_count < fd_count_low_bound)
      fd_count = fd_count_low_bound;
    ...
    if (FD_ISSET(parameters->pipe_from_last, &rset)) {
      ...
      if (rtv != sizeof(ListNode *)) {
        fprintf(
          stderr,
          "outbound: error reading from pipe after %ld bytes: %s\n",
          rtv == -1 ? 0 : rtv,
          strerror(errno)
        );
        break;
      ...
      if (dns_cache_lookup(&cache, c_connection->local_keypair->hostname, &hint, &hint_type)) {
        ...
        ares_getaddrinfo(
          channel, c_connection->local_keypair->hostname, "https", &hints, callback, c_parameter
        );
      }
