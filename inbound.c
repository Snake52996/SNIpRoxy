...
static int cert_callback(
    gnutls_session_t session, [[maybe_unused]] const gnutls_datum_t *req_ca_rdn, [[maybe_unused]] int nreqs,
    [[maybe_unused]] const gnutls_pk_algorithm_t *sign_algos, [[maybe_unused]] int sign_algos_length,
  ...
  if (ret == GNUTLS_E_SUCCESS && type == GNUTLS_NAME_DNS) {
    ...
    *pkey = keypair->key;
    ...
    if (ret != GNUTLS_E_SUCCESS)
      fprintf(stderr, "failed to get SNI: %s\n", gnutls_strerror(ret));
    else
      fprintf(stderr, "unrecognized SNI type: %d\n", type);
    return 1;
  }
}
static int rpc_fd;
...
void *inbound_entry(void *arg) {
  ...
  int ret;
  int nfds;
  struct pollfd fds[] = {{.fd = rpc_fd, .events = POLLIN}, {.fd = listen_socket, .events = POLLIN}};
  ...
  while (loop) {
    ...
    write(parameters->pipe_to_next, &node, sizeof(ListNode *));
